...
// Vérification des arguments en ligne de commande (s'il n'y pas assez de paramettres ou beaucoup d'arguments)
  ...
  else if(argc == 3) // Sinon vérification des arguments en ligne de commande (s'il assez de paramettres)
  {
    ...
    char Impcara; // caracteres imprimables
     ...
     //// Écriture de la sortie standard (stdout)
    for (int i = 0; i < cmpTotalLignes; i++) {
        decimal_value = binary_to_decimal(tableau[i]);
        Impcara = printable_chars[decimal_value];
...
/* Ce programme est un filtre qui a des options prend un fichier binaire en entrée, effectue des opérations de manipulation de bits, 
